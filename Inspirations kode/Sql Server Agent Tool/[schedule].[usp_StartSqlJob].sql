...
IF OBJECT_ID('schedule.usp_StartSqlJob', 'P') IS NOT NULL
  DROP PROCEDURE [schedule].[usp_StartSqlJob];
  ELSE PRINT 'PROCEDURE schedule.usp_StartSqlJob dont exist';

GO

CREATE PROCEDURE [schedule].[usp_StartSqlJob] (
 @p_JobName SYSNAME
...
-- Description:	Start a SQL Agent Job Job
--
-- @result:	0 -> Success
--			1 -> Error
-- Use:
-- DECLARE @v_return INT;
-- EXEC [schedule].[usp_StartSqlJob] @p_JobName='Load Test', @p_RunStatus=@v_return OUTPUT
-- =============================================
	...
	BEGIN
		...
		BEGIN
			-- Enable the Job
			EXEC[msdb].[dbo].[sp_update_job] @job_name = @v_JobName, @enabled = 1
			;
			RAISERROR('Job ''%s'' is enabled', 0, 1, @v_JobName) WITH NOWAIT
			;
		END -- Job is disabled

		EXEC @v_Retval = [msdb].[dbo].[sp_start_job] @job_id = @v_JobId;
		...
		BEGIN		
			...
			SET @p_RunStatus = @v_RunStatus		
			;		
			RAISERROR('Job ''%s'' could not be started', 16, 245, @v_JobName)		
			;		
			RETURN @v_RunStatus;		
		END;		
	...
	END
	ELSE
		RAISERROR('Job ''%s'' is already running', 0, 1, @v_JobName) WITH NOWAIT
	;
...
--DECLARE @v_return INT;
----EXEC [schedule].[usp_StartSqlJob] @p_JobName='CloudAPI Load Test', @p_RunStatus=@v_return OUTPUT
--EXEC [schedule].[usp_StartSqlJob] @p_JobName='DBA.CollectIOStats', @p_RunStatus=@v_return OUTPUT
--PRINT @v_return
