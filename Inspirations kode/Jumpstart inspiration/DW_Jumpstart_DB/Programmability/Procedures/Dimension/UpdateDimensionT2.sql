CREATE PROCEDURE [Dimension].[UpdateDimensionT2]
...
BEGIN
    ...
    VALUES (N'Meta_CreateJob', N'@DataTransferId', NULL, N'@DataTransferId', NULL)
          ,(N'Meta_CreateTime', N'GETDATE()', NULL, N'GETDATE()', NULL)
          ,(N'Meta_UpdateJob', NULL, N'@DataTransferId', NULL, NULL)
          ,(N'Meta_UpdateTime', NULL, N'GETDATE()', NULL, NULL)
          ,(N'Meta_DeleteJob', NULL, NULL, NULL, N'@DataTransferId')
    ...
    EXECUTE [Audit].[DataTransferStart] @SchemaName = ''<DimensionSchemaName>'',
                                        ...
                                        @StoredProcedureName = ''UpdateDimensionT2'',
...
            	INNER JOIN <MapTableFullName> map ON <MapInsertJoin> ;
        ...
        IF OBJECT_ID(''tempdb..#<VersionTableFullName>'') IS NOT NULL DROP TABLE #<VersionTableFullName>
			...
			SELECT <IdColumnName>, ROW_NUMBER () OVER (PARTITION BY <BusinessKeyList> ORDER BY <ValidFromColumnName> ASC) AS Meta_RowNumber
        ...
        CREATE TABLE #<ActionTableFullName> (<IdColumnName> BIGINT PRIMARY KEY, Meta_RowNumber INT, Meta_Action NVARCHAR(10));
        ...
        WHILE (@RowCount > 0) BEGIN
            ...
            USING (
                SELECT s.<IdColumnName>, s.<ValidFromColumnName><StagingColumnList>
                FROM <StagingTableFullName> s
                INNER JOIN <MapTableFullName> m ON <MapMergeJoin>
                INNER JOIN #<VersionTableFullName> x ON x.<IdColumnName> = s.<IdColumnName> AND x.Meta_RowNumber = @RowNumber
            ) sta ON <DimensionJoin>

            WHEN MATCHED AND dim.[Meta_IsCurrent] = 1 AND (<CompareColumnList>)
                THEN UPDATE
                SET dim.[Meta_ValidTo] = sta.Meta_ValidFrom, dim.[Meta_IsCurrent] = 0, dim.[Meta_UpdateTime] = GETDATE(), dim.[Meta_UpdateJob] = @DataTransferId

            WHEN NOT MATCHED BY TARGET
                THEN INSERT (<InsertColumnList>)
                VALUES (<InsertValueList>)
    
            OUTPUT sta.<IdColumnName>, @RowNumber, $action INTO #<ActionTableFullName>;
        ...
        IF @MarkDeletes = 1 BEGIN
            ...
            LEFT JOIN (
                SELECT TOP 1 WITH TIES s.<IdColumnName><StagingColumnList>
                FROM <StagingTableFullName> s
                INNER JOIN <MapTableFullName> m ON <MapMergeJoin>
