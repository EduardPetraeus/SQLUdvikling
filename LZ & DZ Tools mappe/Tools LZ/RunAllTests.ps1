...
$LOGPATH = Join-Path -Path $LOGFOLDER -ChildPath "LOG_08_Databank-LZ_Database_RunAllTests_ps1.log"
...
$databases.name | %{
    ...
    if(($objectId -is [dbnull]) -eq $false){
        ...
        Invoke-Sqlcmd2 -ServerInstance $Server -Database $_ -Query "EXEC [tSQLt].[RunTestClass] 'test_Objekt_LZ'" -Verbose *>&1 | Tee-Object $LOGPATH -Append
