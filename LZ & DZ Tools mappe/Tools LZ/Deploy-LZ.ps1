...
& .\SetRecoveryModelLZ.ps1 -Server $Server
if ($LASTEXITCODE -gt 0) {exit $LASTEXITCODE}

& .\AddPublicationsLZ.ps1 -Server $Server
if ($LASTEXITCODE -gt 0) {exit $LASTEXITCODE}
...
& .\SetAccessRights.ps1 -ServerInstance $Server
if ($LASTEXITCODE -gt 0) {exit $LASTEXITCODE}
...
Set-Location "$PSScriptRoot\MDS"
& .\DeployAllMDSPackages.ps1 -Server $Server
if ($LASTEXITCODE -gt 0) {exit $LASTEXITCODE}
