...
CREATE OR ALTER PROCEDURE [dbo].[AddArticlesLZ] @published_db NVARCHAR(100), @published_pubname NVARCHAR(100)
AS
...
--Find alle tabeller hvor skema er Archive
PRINT 'LZ: Tilføjer artikler for database: "' + @published_db + '" og publicering: "' + @published_pubname + '"'
PRINT ''
...
'WHERE STABLES.[is_ms_shipped] != 1 AND SSCHEMAS.[name] = ''Archive'' ORDER BY [table_name]'
DELETE @TblArticles
