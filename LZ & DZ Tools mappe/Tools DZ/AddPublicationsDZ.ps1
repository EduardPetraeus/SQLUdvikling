...
$LOGPATH = Join-Path -Path $LOGFOLDER -ChildPath "LOG_03_Databank-DZ_Database_0_AddPublications_ps1.log"

get-item AddPublicationsArticlesDZ.sql | %{
    Write-Host -ForegroundColor Green "Executing script: $($_.Name)"
    Write-Output "Executing script: $($_.Name)" | Add-Content -Path $LOGPATH
    Invoke-Sqlcmd2 -InputFile $_.FullName -ServerInstance $ServerInstance -Database tempdb -Verbose *>&1 | Tee-Object $LOGPATH -Append
}

get-item AddPublicationsDZ.sql | %{
    Write-Host -ForegroundColor Green "Executing script: $($_.Name)"
    Write-Output "Executing script: $($_.Name)" | Add-Content -Path $LOGPATH
