...
SELECT TOP(514377)  -- Train rækker: SELECT TOP(514377). Test rækker: SELECT TOP(547057). Dette antal er valgt, da det matcher med afslutningen af ordre for en specifik kunde. Rækkerne skal ind i et Excel ark, så derfor skal der ikke for mange rækker med.
       SAO.[User_Id]                                                                    AS [Kunde Id]
      ,SAO.[Order_Id]                                                                   AS [Ordre Id]		
      ,ISNULL(SAO.[Eval_Set],'Ukendt')                                                  AS [Evaluerings sæt]		
      ...
      ,ISNULL(SOP.[Add_To_Cart_Order],-1)                                               AS [Rækkefølge varer er tilføjet til kurven]
