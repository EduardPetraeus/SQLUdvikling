...
$LOGPATH = Join-Path -Path $LOGFOLDER -ChildPath "LOG_02_Databank-DZ_Database_PublishAllDatabases_ps1.log"
$DIAGPATH = Join-Path -Path $LOGFOLDER -ChildPath "LOG_02_Databank-DZ_Database_PublishAllDatabases_ps1_diag.log"
...
if (Test-Path -Path $profile) {
    ...
    $RawOut = & $SqlPackage /action:publish /sourcefile:"$DacpacFile" /targetservername:$Server /targetdatabasename:"$database" /p:DropObjectsNotInSource="$($DropObjectsNotInSource.ToString())" /p:IncludeCompositeObjects=True /pr:$Profile.FullName /DiagnosticsFile:$DIAGPATH
