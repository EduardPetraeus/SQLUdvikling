...
DELETE @ref
INSERT INTO @ref(id) EXECUTE #ref_id @folder = N'Datawarehouse', @project = N'Salg'

DECLARE @command nvarchar(500) = CONCAT(
	N'/ISSERVER "\"\SSISDB\Datawarehouse\Salg\Master Salg.dtsx\"" /SERVER "\"',
	@@servername,
	N'\""',
...
-- Trigger SQL Agent Job: Staging Dimension

EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Trigger job: Staging - Dimension', 
		@cmdexec_success_code=0, 
		...
		@command=N'EXECUTE (N''EXEC msdb.dbo.sp_start_job @job_name = ''''Staging - Dimension'''', @step_name=''''Master Staging - Dimension'''''')
