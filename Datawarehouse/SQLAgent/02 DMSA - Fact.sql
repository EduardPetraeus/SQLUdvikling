...
DECLARE @jobName NVARCHAR(100) = N'DMSA - Fact'
...
IF @jobId IS NULL BEGIN
	...
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@jobName, 
			...
			@notify_level_page=0, 
			@delete_level=0, 
			@description=N'Loader Fact-tabeller fra Staging til DMSA', 
...
DECLARE @stepName nvarchar(100) = N'Master DMSA Fact' 

DELETE @ref
INSERT INTO @ref(id) EXECUTE #ref_id @folder = N'Datawarehouse', @project = N'DMSA'

DECLARE @command nvarchar(500) = CONCAT(
	N'/ISSERVER "\"\SSISDB\Datawarehouse\DMSA\Master DMSA Fact.dtsx\"" /SERVER "\"',
	@@servername,
	N'\""',
