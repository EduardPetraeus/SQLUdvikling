...
DECLARE @jobName NVARCHAR(100) = N'Staging - Fact'
...
IF @jobId IS NULL BEGIN
	...
	EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=@jobName, 
			...
			@notify_level_page=0, 
			@delete_level=0, 
			@description=N'Loader facttabeller fra archive/views til Staging og starter efterfølgende Master DMSA Fact i DMSA databasen', 
...
-- Step 1 - Kør SSIS pakke: Fact_Ordre

DECLARE @stepName nvarchar(100) = N'Kør Master Staging - Fact' 

DELETE @ref
INSERT INTO @ref(id) EXECUTE #ref_id @folder = N'Datawarehouse', @project = N'Staging'

DECLARE @command nvarchar(500) = CONCAT(
	N'/ISSERVER "\"\SSISDB\Datawarehouse\Staging\Master Staging Fact.dtsx\"" /SERVER "\"',
	@@servername,
	N'\""',
...
-- Trigger SQL Agent Job: DMSA - Fact

EXEC @ReturnCode = msdb.dbo.sp_add_jobstep @job_id=@jobId, @step_name=N'Trigger job: DMSA - Fact', 
		@cmdexec_success_code=0, 
		...
		@command=N'EXECUTE (N''EXEC msdb.dbo.sp_start_job @job_name = ''''DMSA - Fact'''', @step_name=''''Master DMSA Fact'''''')
